...
# Alias: allow --save_steps to control saving cadence (useful for smoke tests)
if getattr(args, "save_steps", None) is not None:
    args.log_every_steps = int(args.save_steps)
...
def main():
    ...
    # UNet is frozen but still heavy; cast it to mixed precision to reduce VRAM usage on single 8G GPUs.
    ...
    # NOTE(Windows): avoid transforms.Lambda(lambda x: x) which is not picklable under spawn-based DataLoader.
    base_transforms = [
    ...
    if args.dataset=='domainnet':
        ...
        # NOTE: keep this import dynamic to avoid Windows/static-analysis issues when the module is not present.
        import importlib
    ...
    split = 'train'
    # Respect user-provided client_num (single-machine smoke tests may set --client_num 1).
    if getattr(args, "client_num", None) is None or args.client_num <= 0:
        args.client_num = 5
    trainloaders = []
    for i in range(args.client_num):
        # Single-machine single-GPU: force num_workers from args (Windows default is 0 to avoid deadlocks).
        if args.dataset == "domainnet":
    ...
    for idx, train_dataloader in enumerate(trainloaders):
        ...
        # Prompt token lengths differ across datasets.
        # Keep them consistent with get_prompt_embeddings() logic.
        if args.dataset == "dermamnist":
            num_prompt_domain = 4
        elif args.dataset == "ucm":
            num_prompt_domain = 3
        ...
        num_prompt_class_tokens = 1
        prompt_class = text_f[:, -1 - num_prompt_class_tokens : -1]
        if args.dataset in ["bloodmnist", "dermamnist", "ucm"]:
            num_prompt_class = [num_prompt_class_tokens for _ in range(len(args.categories))]
        else:
            num_prompt_class = num_prompt_class_tokens
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if global_step%1==0:
                    ...
                    # Some accelerate versions require explicit init_trackers(); guard logging for smoke tests.
        ...
        # Compute statistics on the (scaled) VAE latent distribution.
        # ------------------------------------------------------------
        unet.eval()
        vae.eval()
        latents_all = []
        with torch.no_grad():
            for batch in train_dataloader:
                ...
                latents = latents * 0.18215
                latents_all.append(latents.detach().float().cpu())

        if len(latents_all) > 0:
            latents_all = torch.cat(latents_all, dim=0)
            mean = latents_all.mean(dim=0)
            std = latents_all.std(dim=0)
            torch.save(mean, os.path.join(args.output_dir, f"mean_{idx}.pt"))
            torch.save(std, os.path.join(args.output_dir, f"std_{idx}.pt"))
            print(f"Saved statistics for client {idx}")
        else:
            logger.warning(f"No data found for client {idx}; skipped saving mean/std.")
