...
class DermaMNISTDataset(Dataset):
    def __init__(self, args, domain, split='train', num_shot=-1, root_dir="data/", transform=None, tokenizer=None):
        ...
        if 'niid' in self.split and not 'syn' in self.split:
            ...
            # For synthetic splits, we don't need the original npz file.
            if 'syn' in self.split:
                self.images = None
                self.labels = None
                self.to_few_shot(num_shot)
                return

            npz_path = os.path.join(self.root_dir, "dermamnist_224.npz")
            if not os.path.exists(npz_path):
                if os.environ.get("FEDBIP_SMOKE_DUMMY", "0") == "1":
                    # Minimal dummy dataset for Windows smoke tests.
                    rng = np.random.default_rng(seed=0)
                    split_name = self.split.split("_")[0]
                    n = 32 if split_name == "train" else 16
                    # uint8 HWC images
                    self.images = rng.integers(0, 256, size=(n, 224, 224, 3), dtype=np.uint8)
                    self.labels = rng.integers(0, len(self.categories), size=(n, 1), dtype=np.int64)
                    self.to_few_shot(num_shot)
                    return
                raise FileNotFoundError(
                    f"DermaMNIST data file not found: {npz_path}\n"
                    f"请先准备 dermaMNIST 数据（例如生成/下载 dermamnist_224.npz 到 data/），\n"
                    f"或仅运行冒烟测试时设置环境变量 FEDBIP_SMOKE_DUMMY=1。"
                )

            data = np.load(npz_path)
            split_name = self.split.split("_")[0]
            self.images = data[f"{split_name}_images"]
            self.labels = data[f"{split_name}_labels"]
            self.to_few_shot(num_shot)
            return
